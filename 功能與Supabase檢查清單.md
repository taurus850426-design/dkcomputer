...
正式後台為 **admin.html**（已移除舊版 `admin-v2.html`／`admin.js`／`admin2.js`）。
...
3. `admin3.js`（正式後台 UI：庫存＋記帳／上架／前台／廠商／客戶等）

因此使用 **admin.html** 時會：
- 進入頁面時從 Supabase 拉取 v2 資料（本機已有資料時可能略過覆寫，依 `shared.js` 保護邏輯）
...
| 庫存＋記帳 v2（品項／流水／訂單／支出） | `v2_data` | **任何有載入 shared.js 的頁面**載入時會拉一次；後台切到「庫存＋記帳」再拉一次 | **每次** saveItems / saveLedger / saveOrders / saveExpenses 會觸發 `__syncV2ToSupabase` → 寫入 `v2_data` |
...
### 2. 後台與 Supabase 同步（v2_data）

- [ ] 開 **admin.html**，到 Supabase **Table Editor → v2_data**，確認有 `id = default` 一列，且 `data` 內有 `items`、`ledger`、`orders`、`expenses`。
- [ ] 在後台**新增一筆品項**或**一筆流水**後儲存，重新整理 **v2_data** 表，該筆資料應出現在對應的 `data.items` 或 `data.ledger` 中。
- [ ] 換瀏覽器（或無痕）開 **admin.html**，應能看到剛新增的資料（代表是從 Supabase 拉下來，而不是只存在本機）。

### 3. 後台其他分頁與 Supabase

- [ ] 登入後切到「庫存＋記帳」：若 shared.js 已載入，會再拉一次 v2 資料，品項／流水／訂單／支出應一致。
- [ ] 在後台修改任一 v2 資料並儲存，到 Supabase **v2_data** 應能看到更新；再換裝置開 **admin.html**，應看到同一份資料。
...
4. **載入順序**  
   使用 **admin.html** 時，務必保持 `<script src="./shared.js">` 在 `inventory-ledger.js` 之前，且最後載入 `admin3.js`，否則 v2 可能無法同步到 Supabase。
